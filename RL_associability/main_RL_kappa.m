...
function [] = main_RL_kappa(subject_id)
    ...
    addpath('/media/labs/rsmith/lab-members/clavalley/MATLAB/spm12/');
    addpath('/media/labs/rsmith/lab-members/clavalley/MATLAB/spm12/toolbox/DEM/'); 
    
    %% Fit each subject and keep the list of Free energy, 
    % all_sub_ids = readtable('/Volumes/labs/rsmith/lab-members/nli/CPD_updated/T475_list.csv');
    % all_sub_ids = table2cell(all_sub_ids);
    % data_dir = "/Volumes/labs/rsmith/lab-members/nli/CPD_updated/Individual_file_mat";
    % data_dir = "/media/labs/rsmith/lab-members/nli/CPD_updated/Individual_file_mat";
    F_RL_kappa_model = [];
    LL_RL_kappa_model = [];
    ActionAccu_RL_kappa_model = [];
    Accuracy_RL_kappa_model = [];
    % error_messages = {};
    ...
    DCM.MDP.kappa_prior = kappa_prior;
    DCM.MDP.eta = eta;
    %DCM.field  = {'reward_lr' 'inverse_temp' 'reward_prior' 'kappa_prior' 'eta'}; % Parameter field
    DCM.field  = {'reward_lr' 'inverse_temp' 'reward_prior' 'kappa_prior' 'eta'}; 
    DCM.U = MDP.trials;
    DCM.Y = 0;
    CPD_fit_output= CPD_RL_kappa_fit(DCM);
    %output_params = [1/(1+exp(-CPD_fit_output.Ep.reward_lr)) 1/(1+exp(-CPD_fit_output.Ep.latent_lr)) 1/(1+exp(-CPD_fit_output.Ep.new_latent_lr)) exp(CPD_fit_output.Ep.inverse_temp) CPD_fit_output.Ep.reward_prior_1 CPD_fit_output.Ep.reward_prior_2 CPD_fit_output.Ep.reward_prior_3];
    output_params = [1/(1+exp(-CPD_fit_output.Ep.reward_lr)) exp(CPD_fit_output.Ep.inverse_temp) CPD_fit_output.Ep.reward_prior 1/(1+exp(-CPD_fit_output.Ep.kappa_prior)) 1/(1+exp(-CPD_fit_output.Ep.eta))];
    ...
    params.kappa_prior = output_params(4);
    params.eta = output_params(5);
    
    % rerun model a final time
    L = 0;
    %action_probabilities = CPD_latent_model(params, trials, 0); 
    ...
    F_RL_kappa_model = [F_RL_kappa_model; CPD_fit_output.F];
    LL_latent_model = [LL_RL_kappa_model; L];
    ActionAccu_RL_kappa_model = [ActionAccu_RL_kappa_model; action_accuracy];
    Accuracy_RL_kappa_model = [Accuracy_RL_kappa_model; accuracy];

    % file_name = sprintf('/Volumes/labs/rsmith/lab-members/nli/CPD_updated/Output_Individual_mat/%s_individual.mat', subject_id);
    file_name = sprintf('/media/labs/rsmith/lab-members/nli/CPD_updated/Output_Individual_mat_for_all_models/Output_Individual_mat_RL_kappa/%s_individual_RL_k.mat', subject_id);
    save(file_name)
    ...
    output.kappa_prior = output_params(4);
    output.eta = output_params(5);
  ...
  % end
  filename = sprintf('/media/labs/rsmith/lab-members/nli/CPD_updated/Output_Individual_csv_for_all_models/Output_Individual_csv_RL_kappa/%s_individual_RL_k.csv', subject_id);
