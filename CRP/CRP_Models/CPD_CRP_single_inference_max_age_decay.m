function action_probs = CPD_CRP_single_inference_max_age_decay(params, trials)
rng(1)
choices = [];
    ...
    for t = 1:min(trial_length, 3)
        ...
        if t == 1 % this is the first choice 
            ...
            if trial_length == 1 
                ...
                prediction_error = learning_rate*(outcome - latent_state_rewards(new_CRP_idx, :));
                latent_state_rewards(new_CRP_idx, :) = latent_state_rewards(new_CRP_idx, :) + prediction_error;
                ...
                latent_state_assignment(trial, new_CRP_idx) = 1;
                decayed_counts = updated_decayed_counts(decayed_counts, latent_state_assignment, gamma);
                latent_state_counts = decayed_counts(trial, :);
...
function decayed_counts = updated_decayed_counts(decayed_counts, latent_state_assignment, gamma)
    num_latent_states = size(latent_state_assignment, 2);

    if isempty(decayed_counts)
        decayed_counts = zeros(0, num_latent_states);
    elseif size(decayed_counts, 2) < num_latent_states
        num_extra_columns = num_latent_states - size(decayed_counts, 2);
        decayed_counts = [decayed_counts, zeros(size(decayed_counts, 1), num_extra_columns)];
    end


    num_trials = size(latent_state_assignment, 1);
    for trial_point = size(decayed_counts, 1) + 1 : num_trials
        new_counts = zeros(1, num_latent_states); 
        
        for t_prime = 1:trial_point
            decay_weight = gamma^(trial_point - t_prime);
            for latent_state = 1:num_latent_states
                new_counts(latent_state) = new_counts(latent_state) + ...
                    latent_state_assignment(t_prime, latent_state) * decay_weight;
            end
        end
        
        decayed_counts = [decayed_counts; new_counts];
    end
end
...
%     num_latent_states = size(latent_state_assignment, 2);
% 
%     if size(decayed_counts, 2) < num_latent_states
%         num_extra_columns = num_latent_states - size(decayed_counts, 2);
%         decayed_counts = [decayed_counts, zeros(size(decayed_counts, 1), num_extra_columns)];
%     end
% 
%     new_counts = zeros(1, num_latent_states);
% 
%     for trial_point = 1: size(latent_state_assignment, 1)
%         for t_prime = 1: trial_point
%             decay_weight = gamma^(trial_point - t_prime);
%             for latent_state = 1: num_latent_states
%                 new_counts(latent_state) = new_counts(latent_state) + latent_state_assignment(t_prime, latent_state) * decay_weight;
%             end
%         end
%     end
%     decayed_counts = [decayed_counts; new_counts];
