   ...
   % load(filename);
    ...
    for i = 1:length(outer_fit_list)
        ...
        for j = 1:length(inner_fit_list)
            ...
            % specify that action_prob maps to drift/bias or leave blank so
            % that the params will be fit as free parameters
            if DCM.use_DDM
                ddm_mappings = {
            ...
            for mapping_index=1:length(ddm_mappings)
               ...
               % DCM.MDP.reward_prior = reward_prior;
                ...
                elseif strcmp(inner_fit_list{j}, 'basic') || strcmp(inner_fit_list{j}, 'temporal')
                    ...
                    file_name = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/latent_model/ind_mat/%s_individual_%s_%s_forget_%s.mat'], subject_id, func2str(DCM.model), decay_type, ddm_mapping_string);
                    filename = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/latent_model/threshold/%s_individual_%s_%s_forget_%s.csv'], subject_id, func2str(DCM.model), decay_type, ddm_mapping_string);
                end
