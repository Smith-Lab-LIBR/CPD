...
function [action_probs, choices] = latent_single_inference_max_simulated(params, trials, decay_type)
...
%latent_learning_rate = 0.3;
%latent_learning_rate_new = 0.2;
    ...
    while trial_over == 0
        ...
        if t == 1
            ...
            if choice == result
                ...
                [latent_states_distribution, temporal_mass, max_evidence] = adjust_latent_distribution(latent_states_distribution, latent_state_rewards, choice, latent_learning_rate,0, 1,  timestep, temporal_mass, decay_type);
                [new_latent_states_distribution, new_temporal_mass, max_evidence_new] = adjust_latent_distribution(new_latent_states_distribution, new_latent_state_rewards, choice, latent_learning_rate,latent_learning_rate_new, 1, timestep, new_temporal_mass, decay_type);
                ...
                prediction_error = learning_rate * c*(outcome - latent_state_rewards(max_evidence,:));
                prediction_error_next = learning_rate * (outcome - new_latent_state_rewards(max_evidence_new,:));
    
                % update latent state reward predictions weighted by latent
                % state distribution
                latent_state_rewards(max_evidence,:) = latent_state_rewards(max_evidence,:) +  prediction_error;
                new_latent_state_rewards(max_evidence_new,:) = new_latent_state_rewards(max_evidence_new,:) + prediction_error_next;
         ...
         t = t + 1;
            ...
            if ~ isempty(result) % some entries are weird and dont ever have the correct result. These might need to be discarded. TODO double check this later
                ...
                new_latent_state_rewards(max_evidence_new,columnIndices) = new_latent_state_rewards(max_evidence_new,columnIndices) +  prediction_error_next;
                timestep = timestep + 1;
             end
           trial_over = 1;
           t = t+1;
