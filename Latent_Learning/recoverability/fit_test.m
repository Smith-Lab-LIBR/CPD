...
for t = 1:length(trials)
    ...
    first_choice = trial;
    if size(action_probabilities{t},1) > 1 
        second_choice = trial;
        L = L + log(action_probabilities{t}(1,trial) + eps)/2;
        L = L + log(action_probabilities{t}(2,trial ) + eps)/2;
        average_action_probability = average_action_probability + action_probabilities{t}(1,trial);
        [maxi, idx_first] = max(action_probabilities{t}(1,:));
        if idx_first == first_choice 
            average_accuracy = average_accuracy + 1;
        end
        accuracy_count = accuracy_count +1;
        count = count + 1;
        average_action_probability = average_action_probability + action_probabilities{t}(2,trial);
        [maxi, idx_first] = max(action_probabilities{t}(2,:));
         if idx_first == second_choice
            average_accuracy = average_accuracy + 1;
         end
        count = count + 1;
         accuracy_count = accuracy_count +1;
    else
        Likelihood = log(action_probabilities{t}(1,first_choice) + eps);
        L = L + Likelihood;
        ap = action_probabilities{t}(1,first_choice);
        [maxi, idx_first] = max(action_probabilities{t}(1,:));
         if idx_first == first_choice 
            average_accuracy = average_accuracy + 1;
        end
