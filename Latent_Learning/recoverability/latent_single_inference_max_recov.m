...
%% for testing %%
    ...
    latent_learning_rate_new = 0.5;%params.new_latent_lr;
...
for trial = 1:length(trials)
    ...
    true_actions = simulated_trials{trial};
    ...
    for t = 1:min(trial_length, 3)      
        ...
        if t == 1
            ...
            if ~ isempty(result) % some entries are weird and dont ever have the correct result. These might need to be discarded. TODO double check this later
                previous_result_idx = true_actions(t-1,1) + 1; % access the action taken in the previous trial; the value in the first column of the row corresponding to the previous trial index (t-1)
                ...
                latent_state_rewards(max_evidence,columnIndices) = latent_state_rewards(max_evidence,columnIndices) +  prediction_error;
        ...
        %if t == trial_length && idx_new == length(new_latent_states_distribution)
        if t == trial_length && new_prob > latent_learning_rate_new
