...
function [] = main_RL_k(subject_id)
    ...
    decay = 0.8;
    eta = 0.5;
   outer_fit_list = {@CPD_RL_kappa_model};
    ...
    for i = 1:length(outer_fit_list)
        ...
        for j = 1:length(inner_fit_list)
            ...
            if strcmp(j, "vanilla")
                 ...
                 file_name = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/rl_k/ind_mat/%s_individual_%s.mat'], subject_id, func2str(DCM.model));
                 filename = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/rl_k/ind_csv/%s_individual_%s.csv'], subject_id, func2str(DCM.model));
            else
                DCM.MDP.decay = decay;
                DCM.field  = {'reward_lr' 'inverse_temp' 'reward_prior' 'kappa_prior', 'decay', 'eta' }; % Parameter field
                file_name = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/rl_k/ind_mat/%s_individual_%s_%s.mat'], subject_id, func2str(DCM.model), decay_type);
                filename = sprintf([root 'rsmith/lab-members/rhodson/CPD/CPD_results/rl_k/ind_csv/%s_individual_%s_%s.csv'], subject_id, func2str(DCM.model), decay_type);
            end
            ...
            if isfield(CPD_fit_output.Ep, 'kappa_prior')
                params.kappa_prior = 1/(1+exp(-CPD_fit_output.Ep.kappa_prior));
            end
            if isfield(CPD_fit_output.Ep, 'decay')
                params.decay = 1/(1+exp(-CPD_fit_output.Ep.decay)); 
            end
            if isfield(CPD_fit_output.Ep, 'eta')
                params.eta = 1/(1+exp(-CPD_fit_output.Ep.eta)); 
            end
            ...
            output.kappa_prior = params.kappa_prior;
            output.eta= params.eta;
