...
function DCM = CPD_RL_kappa_fit(DCM)
...
for i = 1:length(DCM.field)
    ...
    if ALL
        ...
        elseif strcmp(field,'reward_prior')
            ...
            pC{i,i}    = 0.5;        
        elseif strcmp(field, 'kappa_prior')
            pE.(field) = log(DCM.MDP.kappa_prior/(1-DCM.MDP.kappa_prior));
            pC{i,i}    = 0.5;
        elseif strcmp(field, 'eta')
            pE.(field) = log(DCM.MDP.eta/(1-DCM.MDP.eta)) ;
            pC{i,i}    = 0.5;         
        else
            pE.(field) = 0;      
